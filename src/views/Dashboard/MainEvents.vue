      ...
      <div class="mt-8 p-8 shadow-xs rounded-lg">
        <div class="grid lg:grid-cols-2 lg:gap-5 row-gap-5">
          <div class="flex items-center">
            ...
            <input class="bg-gray-100 font-light ml-5 focus:outline-none focus:shadow-outline border border-gray-300 rounded-full py-2 pl-4 lg:px-4 block w-full appearance-none leading-normal" type="string" placeholder="Title" />
          </div>
          <div class="flex items-center">
            <h1 class="text-base font-light text-gray-700 tracking-wide">Time</h1>
            <input class="bg-gray-100 font-light ml-5 focus:outline-none focus:shadow-outline border border-gray-300 rounded-full py-2 pl-4 lg:px-4 block w-full appearance-none leading-normal" type="time" placeholder="Time" />
          </div>
          <div class="flex items-center justify-between">
            <h1 class="text-base font-light text-gray-700 tracking-wide flex">Start Date</h1>
            <input class="bg-gray-100 font-light ml-10 focus:outline-none focus:shadow-outline border border-gray-300 rounded-full py-2 pl-4 lg:px-4 block appearance-none leading-normal" type="date" placeholder="jane@example.com" />
          </div>
          <div class="flex items-center justify-between">
        ...
        <div class="flex items-center mt-5">
          <h1 class="text-base font-light text-gray-700 tracking-wide">Title</h1>
          <input class="bg-gray-100 font-light ml-5 focus:outline-none focus:shadow-outline border border-gray-300 rounded-full py-2 pl-4 lg:px-4 block w-full appearance-none leading-normal" type="email" placeholder="jane@example.com" />
        </div>
        <div class="mt-5 flex lg:flex-row flex-col">
          <div @click="$refs.file.click()" class="h-32 border w-32 rounded-lg flex justify-center items-center relative">
            <svg class="fill-current text-gray-400 w-8 h-8" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
            ...
            <input type="file" ref="file" @change="change" class="hidden" name id />
            <div class="text-xs text-gray-600 absolute bottom-0 -mb-2 bg-white px-2">Upload Image</div>
          </div>
          <textarea placeholder="Description" type="text" class="h-32 flex-grow text-sm p-2 lg:mt-0 mt-5 rounded-lg border lg:ml-10 focus:outline-none focus:shadow-outline" />
        </div>
      </div>
          ...
          <carousel :per-page="page" :centerMode="false" :navigationEnabled="width" :paginationEnabled="false" :loop="true">
            <slide v-for="(img, index) in images" :key="index">
              <div :class="`box-${index}`" class="rounded  shadow-xl  mx-4 p-6">
                <img style="filter:brightness(90%)" class="h-64 w-full object-cover" :src="img" alt />
                <div class="flex  justify-between  items-center">
                  <div>
                    <div class=" text-xs uppercase font-bold text-gray-800 mt-2">
                      Youth Ablaze
                    </div>
                    <div class=" text-xs  capitalize text-gray-600 leading-3">
                      Created: 22/06/2020
                    </div>
                  </div>
                  <div>
                    <svg class="w-6 h-6 text-gray-600 fill-current mt-1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
                      <path
